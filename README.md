...
Subscribers (optional), on Resend — see `docs/RESEND.md`: secret `RESEND_API_KEY` (full access) and variables `RESEND_SEGMENT_ID`, `RESEND_TOPIC_DAILY`, `RESEND_TOPIC_WEEKLY`, `MAIL_FROM`, `MAIL_REPLY_TO`, `MAIL_POSTAL_ADDRESS`, `SUBSCRIBE_URL`. Sign-up is double opt-in through the Cloudflare Worker in `worker/subscribe` (deployed by the `mail-setup` workflow, which also creates the Resend segment and topics); the form renders only when `SUBSCRIBE_URL` is set. `scripts/mail.js` sends the daily edition to the daily topic and the Monday review to the weekly topic, and refuses to send until `MAIL_POSTAL_ADDRESS` is set (the footer needs it). Episodes are served from the R2 custom domain; the feed's enclosure URLs go through [OP3](https://op3.dev) (`https://op3.dev/e,pg=<podcast:guid>/…`) for open, IAB-style download stats — public at `https://op3.dev/show/<guid>`. The show's `podcast:guid` is pinned in `scripts/lib.js`.
