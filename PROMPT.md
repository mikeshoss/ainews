...
- **Intro (required, checked by the validator)**: the first line says the date exactly as `Friday, 11 September 2026` and the show name and presenter exactly once: "This is The AI Edge, presented by Epilogue." Each host introduces themselves by name in their own line ("I'm Maya." / "And I'm Theo."). One line discloses that the voices are AI. Then the three things that matter most, from the summary. **"Epilogue" must not appear anywhere else in the script** — no plugs, no sign-off mention.
