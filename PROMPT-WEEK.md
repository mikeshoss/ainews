...
- **Propose at most one new storyline per week**, only if the admission test holds: it could plausibly be settled by a future event, and it has either 3+ developments across 2+ weeks or one development that clears the daily significance bar. Create `storylines/<id>.json` with `"status": "proposed"` and a `proposed_note` saying why. **A proposed storyline is never published**: the build keeps it off the site and puts it in the editor's Monday email instead; the editor promotes it to `live` (or renames/merges it). The cap is **12 live** — do not promote past it.
