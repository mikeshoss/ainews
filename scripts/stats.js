...
function podcastHealth(row) {
  ...
  if (!(row.edition && row.edition.script)) return { state: 'narration', label: 'narrated', bad: false, detail: `No two-host script was committed for ${row.date}, so the episode is the code-read narration. That is the fallback working as intended — the run either never produced a script or deleted one that failed its fact-check.` };
  return {
    state: 'wrong', label: 'narrated', bad: true,
    detail: `The published audio for ${row.date} is the single-voice narration, but data/${row.date}.script.json exists — a two-host script was written and not used. That means the audio was generated by a build that ran before the script was committed. To replace it: run the deploy workflow with force_date=${row.date} and a new label (the narrated version is kept).`,
  };
}
...
function printTable(rows, weeks, snaps, ga, gaError, op3Error) {
  ...
  const cols = [['Date', 10], ['Items', 5, 1], ['Run', 5, 1], ['Tools', 5, 1], ['Claude $', 8, 1], ['TTS $', 6, 1], ['Total $', 7, 1], ['Episode', 14], ['Ep all', 6, 1], ['New dl', 6, 1], ['Users', 5, 1], ['Views', 5, 1], ['Clicks', 6, 1], ['Notes', 0]];
...
<td>${(() => { const h = podcastHealth(r); if (h.state === 'none') return n(null);
  if (!h.detail) return `<span class="chip ok">${esc(h.label)}</span>`;
  return `<span class="chip ${h.bad ? 'bad' : 'meh'}" title="${esc(h.detail)}">${h.bad ? '&#9888; ' : ''}${esc(h.label)}</span>`; })()}</td><td class="r">${n(r.podcast && r.podcast.episode_downloads)}</td><td class="r">${n(r.downloads_today)}</td>
<td class="r">${n(r.site && r.site.users)}</td><td class="r">${n(r.site && r.site.views)}</td><td class="r">${n(r.site && r.site.clicks)}</td>
<td>${[r.run && !r.run.email_sent && r.edition ? 'no email' : '', r.run && r.run.sessions > 1 ? `${r.run.sessions} sessions on this trace` : ''].filter(Boolean).join(', ')}</td></tr>${(() => { const h = podcastHealth(r); return h.bad ? `\n<tr class="why"><td colspan="17"><div class="whybox bad">${esc(h.detail)}</div></td></tr>` : ''; })()}`).join('\n');
...
.chip{display:inline-block;font-size:.8rem;border:1px solid var(--line);border-radius:99px;padding:2px 9px;white-space:nowrap;text-decoration:none;color:var(--muted)}
span.chip{color:var(--fg)}
.chip.bad{color:#c0392b;border-color:#c0392b66;background:#c0392b14;font-weight:600}
/* Shown, not toggled: it only renders on a day that actually went wrong, and the artifact viewer does
   not let the page drive its own state. */
.why td{padding:0 10px 12px}
.whybox{white-space:normal;max-width:80ch;border-left:3px solid var(--line);padding:8px 12px;color:var(--muted);font-size:.9rem;line-height:1.55}
.whybox.bad{border-left-color:#c0392b;color:var(--fg)}

</style>
<h1 id="top">AI Edge Briefing — private stats</h1>
